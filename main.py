...
async def generate_industry_based_answer(question, keyword, industry, current_keywords, reason: Optional[str] = None):
    """산업별/직무별 관점 분석 기반 답변 생성 (긍정적 분석) - 간결하게 변경"""
    try:
        industry_context = {
        ...
        **답변 형식:**
        ...
        completion = openai_client.chat.completions.create(
            ...
            max_tokens=500, # 최대 토큰을 줄여서 길이 제한 강화 (250자면 대략 200-300 토큰 내외)
            temperature=0.3
        )
...
async def generate_comparison_answer(question, keywords, perspective_role: Optional[str] = None, reason: Optional[str] = None):
    """키워드들을 비교 분석 답변 생성 (직무/산업 관점 포함) - 간결하게 변경"""
    try:
        role_context = ""
        if perspective_role:
        ...
        **답변 형식:**
        ...
        completion = openai_client.chat.completions.create(
            ...
            max_tokens=500, # 최대 토큰을 줄여서 길이 제한 강화
            temperature=0.3
        )
